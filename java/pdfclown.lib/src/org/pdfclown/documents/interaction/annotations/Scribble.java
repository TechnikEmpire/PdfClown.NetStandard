  ...
  extends Markup<Scribble>
{
  ...
  {
    super(page, PdfName.Ink, new Rectangle2D.Double(), text);
    setPaths(paths);
    setColor(color);
  }
  ...
  */
    ...
    )
  {
    ...
    for(List<Point2D> path : value)
    {
      ...
      {
        if(box == null)
        {box = new Rectangle2D.Double(point.getX(), point.getY(), 0, 0);}
        else
        {box.add(point);}
